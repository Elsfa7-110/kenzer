...
class Kenzer(object):
    ...
    def __init__(self):
        ...
        time.sleep(3)
        self.modules = ["monitor", "ignorenum", "subenum", "webenum", "servenum", "urlheadenum", "headenum", "socenum", "conenum", "dnsenum", "portenum", "asnenum", "urlenum", "favscan",
    ...
    def ignorenum(self):
        for i in range(2, len(self.content)):
            ...
            self.enum = enumerator.Enumerator(
                domain, _kenzerdb, _kenzer, _github, dtype)
    ...
    def subenum(self):
        for i in range(2, len(self.content)):
            ...
            self.enum = enumerator.Enumerator(
                self.content[i].lower(), _kenzerdb, _kenzer, dtype, _github)
    ...
    def urlenum(self):
        for i in range(2, len(self.content)):
            ...
            self.enum = enumerator.Enumerator(
                self.content[i].lower(), _kenzerdb, _kenzer, _github, dtype)
            self.mergekenz(self.content[i].lower())
        ...
        for i in range(2, len(self.content)):
            ...
            if validators.domain(self.content[i].lower()) == True or self.content[i].lower() == "monitor":
                ...
                try:
                    ...
                    continue
            self.sendMessage(
                "[idscan - #({0}/{1})] {2}".format(i-1, len(self.content)-2, self.content[i].lower()))
            self.scan = scanner.Scanner(
                self.content[i].lower(), _kenzerdb, dtype, _kenzer)
            self.mergekenz(self.content[i].lower())
