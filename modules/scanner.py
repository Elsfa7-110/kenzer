...
class Scanner:
    ...
    def nuclei(self, template, hosts, output):
        ...
        os.system("nuclei -project -project-path {4}/nuclei -stats -retries 2 -bulk-size 100 -rate-limit 50 -t {3}nuclei/{0}{5} -timeout 10 -l {1} -o {2}".format(
    ...
    def jaeles(self, template, hosts, output):
        ...
        if len(self.severity) > 0:
            severity = self.severity
        os.system("jaeles scan --retry 2 --no-background -c 50 --rootDir {3}jaeles/ -s {3}jaeles/{0}/{5} --timeout 10 -U {1} -O {2} -o {4}/jaeles --no-db --chunk true ".format(
            template, hosts, output, self.templates, self.path, self.severity))
        return
    ...
    def subscan(self):
        ...
        self.nuclei(
            "subscan/dns/subdomain-takeover-dns-wildcards.yaml", subs, output)
    ...
    def favscan(self):
        ...
        if(os.path.exists(out)):
            os.system("mv {0} {0}.old".format(out))
        os.system(
            "favinizer -d {2}/favinizer.yaml -t 10 -T 50 -l {0} -o {1}".format(subs, out, self.templates))
        if(os.path.exists(out)):
            with open(out, encoding="ISO-8859-1") as f:
                line = len(f.readlines())
        else:
    ...
    def portscan(self):
        ...
        if(os.path.exists(out)):
            os.system("mv {0} {0}.old".format(out))
        os.system("sudo NXScan --only-scan -l {0} -o {1} -T {2}/nmap-bootstrap.xsl".format(
            subs, path+"/nxscan", self.templates))
    ...
    def vizscan(self):
        ...
        os.system(
            "cp {0}/aquatone_report.html {1}".format(path+"/aquatone", out))
        os.system(
            "sed -i 's+screenshots/+aquatone/screenshots/+g' {0}".format(out))
